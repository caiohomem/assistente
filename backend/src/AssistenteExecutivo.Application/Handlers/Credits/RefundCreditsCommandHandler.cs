...
public class RefundCreditsCommandHandler : IRequestHandler<RefundCreditsCommand, RefundCreditsResult>
{
    private readonly ICreditWalletRepository _walletRepository;
    private readonly IClock _clock;
    private readonly IUnitOfWork _unitOfWork;

    public RefundCreditsCommandHandler(
    ...
    public async Task<RefundCreditsResult> Handle(RefundCreditsCommand request, CancellationToken cancellationToken)
    {
        // Validar OwnerUserId
        ...
        var existingTransaction = wallet.Transactions
            .FirstOrDefault(t => t.IdempotencyKey == idempotencyKey && 
                                 t.Type == Domain.Enums.CreditTransactionType.Refund);

        bool wasIdempotent = existingTransaction != null;

        // Se já existe transação Refund com essa idempotency key, retornar resultado idempotente
        if (wasIdempotent)
        {
            return new RefundCreditsResult
            {
        ...
        // Executar Refund no domínio (valida idempotência internamente)
        wallet.Refund(creditAmount, idempotencyKey, request.Purpose, _clock);
