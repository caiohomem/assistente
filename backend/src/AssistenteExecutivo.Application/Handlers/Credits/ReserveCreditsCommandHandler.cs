...
public class ReserveCreditsCommandHandler : IRequestHandler<ReserveCreditsCommand, ReserveCreditsResult>
{
    ...
    public async Task<ReserveCreditsResult> Handle(ReserveCreditsCommand request, CancellationToken cancellationToken)
    {
        // Validar OwnerUserId
        ...
        // Se já existe transação com essa idempotency key, retornar resultado idempotente
        if (wasIdempotent)
        {
            return new ReserveCreditsResult
            {
        ...
        // Executar Reserve no domínio (valida saldo e idempotência internamente)
        wallet.Reserve(creditAmount, idempotencyKey, request.Purpose, _clock);
