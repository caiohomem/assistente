...
public class ActivateAgreementCommandHandler : IRequestHandler<ActivateAgreementCommand, Unit>
{
    ...
    public async Task<Unit> Handle(ActivateAgreementCommand request, CancellationToken cancellationToken)
    {
        ...
        _rulesService.EnsureCanActivate(agreement);

        agreement.Activate(_clock);
