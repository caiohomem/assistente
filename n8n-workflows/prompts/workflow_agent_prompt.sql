...
Never output Manual trigger in specJson.

For email actions, use exactly this structure (only edit the expressions/values as needed):
...
Respond only with JSON specs following the schema above. Make sure the JSON body sent to the webhook contains all fields referenced in parameters (from, to, subject, text, html if needed).$prompt$,
        v_created_at,
        v_updated_at
    );
END $$;
