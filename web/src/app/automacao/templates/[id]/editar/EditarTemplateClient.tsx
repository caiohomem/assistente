...
import { updateTemplateClient } from "@/lib/api/automationApiClient";
import { TemplateType } from "@/lib/types/automation";

interface EditarTemplateClientProps {
  templateId: string;
  initialData: {
    name: string;
    type: TemplateType;
...
export function EditarTemplateClient({ templateId, initialData }: EditarTemplateClientProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    placeholdersSchema: initialData.placeholdersSchema || "{}",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateTemplateClient(templateId, {
      ...
      router.push(`/automacao/templates/${templateId}`);
      router.refresh();
    } catch (error) {
      console.error("Erro ao atualizar template:", error);
      setErrors({
        submit: error instanceof Error ? error.message : "Erro ao atualizar template",
      });
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          O tipo do template não pode ser alterado após a criação.
        </p>
      </div>
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Apenas templates ativos podem ser usados na criação de documentos.
        </p>
      </div>
