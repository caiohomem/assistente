...
import { updateDraftClient, listTemplatesClient, listLetterheadsClient } from "@/lib/api/automationApiClient";
...
import { DocumentType, DraftStatus } from "@/lib/types/automation";

interface EditarRascunhoClientProps {
  draftId: string;
  initialData: {
...
export function EditarRascunhoClient({ draftId, initialData }: EditarRascunhoClientProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    letterheadId: initialData.letterheadId,
  });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const canEdit = initialData.status === DraftStatus.Draft;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canEdit) {
      setErrors({
        submit: "Rascunhos enviados não podem ser editados",
      });
      return;
    }
    ...
    setLoading(true);
    try {
      await updateDraftClient(draftId, {
      ...
      router.push(`/automacao/rascunhos/${draftId}`);
      router.refresh();
    } catch (error) {
      console.error("Erro ao atualizar rascunho:", error);
      setErrors({
        submit: error instanceof Error ? error.message : "Erro ao atualizar rascunho",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          type="text"
          value={getDocumentTypeLabel(initialData.documentType)}
          disabled
          className="w-full rounded-md border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400 px-4 py-2 text-sm cursor-not-allowed"
        />
        ...
        {loadingData ? (
          ...
          <select
            ...
            className={`w-full rounded-md border border-zinc-300 dark:border-zinc-700 ${
              !canEdit ? "bg-zinc-100 dark:bg-zinc-700 cursor-not-allowed" : "bg-white dark:bg-zinc-800"
            } text-zinc-900 dark:text-zinc-100 px-4 py-2 text-sm focus:border-indigo-500 dark:focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500`}
          >
      ...
      <div>
        ...
        <textarea
          ...
          className={`w-full rounded-md border ${
            ...
            !canEdit ? "bg-zinc-100 dark:bg-zinc-700 cursor-not-allowed" : "bg-white dark:bg-zinc-800"
          } text-zinc-900 dark:text-zinc-100 px-4 py-2 text-sm focus:border-indigo-500 dark:focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono`}
