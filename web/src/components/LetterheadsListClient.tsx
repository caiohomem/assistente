...
import { NovoPapelTimbradoClient } from "@/app/automacao/papeis-timbrados/novo/NovoPapelTimbradoClient";
import { ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";

interface LetterheadsListClientProps {
  initialData?: ListLetterheadsResult;
}

export function LetterheadsListClient({ initialData }: LetterheadsListClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const showNewLetterhead = searchParams.get("novo") === "true";
  
  const [loading, setLoading] = useState(!initialData);
  const [letterheads, setLetterheads] = useState<Letterhead[]>(initialData?.letterheads || []);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {loading && letterheads.length === 0 ? (
          ...
          <div className="glass-card p-12 text-center">
            <p className="text-muted-foreground">Nenhum papel timbrado encontrado.</p>
            <button
              onClick={() => router.push("/documentos?tab=letterheads&novo=true")}
